...
- Observation: 
	...
	- This implies the Max of L(w,b,α) wrt α will be less than or equal to ||w||<sup>2</sup>/2 --> lets call this the objective function 
...
- Our this is called the primal from, we will convert this to the dual form.
- Primal: min<sub>w,b</sub>(max<sub>α</sub>(L(w,b,α)))
- Dual form: max<sub>α</sub>(min<sub>w,b</sub>(L(w,b,α)))
	- Both primal and dual solution will be the same if
...
- If we have a f(x1, x2), that gives the dot product of x1,x2 in the higher dimension in terms of x1,x2, then we can use this function
	- These types of functions are called kernel functions
	- The kernel also represents the similarity of two vectors, choosing a kernel requires domain knowledge and knowledge about how kernels work
#### 4.1 Dual problem using kernel and soft margin
...
- To find if a function if positive semi definite:
	- Find the Hessian of the function
	- x<sup>T</sup>Hx >=0 for all non-zero vectors x
- How to find the Hessian matrix H of a function?
	- Hessian matrix is the second order partial derivative of f wrt to all its inputs ![](./Attachments/hessian_matrix.png) 
