...
- The state can be maintained using cookies
	...
	- The server has the ability to set or delete the cookies in a client
...
- Sequence of message exchanged between client and server
	...
	- Server checks if client request has cookies
		- If no cookie, and set-cookie header to http response
...
- The client sends the request to this third part server, with a referrer tag amazon
...
- Let's say instagram also uses the same tracker. Now that the client already has the cookie of the third party server(happened when visiting amazon), the client will send this cookie
	- The tracker can now add a new entry saying that this client with this cookie has visited instagram.
- Preventing tracking:
